...
def get_start_block():
    try:
        with open('data_miner/last_processed_block.txt', 'r') as f:
            return int(f.read().strip())
    except FileNotFoundError:
        return 0

def save_last_block(block):
    with open('data_miner/last_processed_block.txt', 'w') as f:
        f.write(str(block+1))
...
def save_erc20_transactions(data, wallet_name):
    ...
    file_path = os.path.join('data_miner', 'raw_txs', f'${wallet_name}', 'token.csv')
...
def save_internal_transactions(data, wallet_name):
    ...
    file_path = os.path.join('data_miner', 'raw_txs', f'${wallet_name}', 'internal.csv')
